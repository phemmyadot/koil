...
VCPO) — each is a trade-for-trade mirror of its `backend/` Python module:
- `pines/strategy_d_volatility_exhaustion.pine` — the shipped/production strategy: buys
  ...
  strategy mirror. (Filename predates the backend's later expansion — despite the `_d_`, this
  is VEXH, not "Strategy D".)
- `pines/vcp.pine` — "VCP Master", ATR-compression + volume-confirmed breakout with a
  multi-tier stop/breakeven/trail and partial take-profit. Mirrors `backend/strategy_vcp.py`.
- `pines/vcpo.pine` — "VCPO", same ATR-compression breakout as VCP Master but without the
  volume-confirmation gate. Mirrors `backend/strategy_vcpo.py`.
...
- `strategy_a_adaptive_pullback.pine` / `strategy_b_adx_scaled_trail.pine` — adaptive
  KAMA-based pullback variants, deprecated (no `backend/` module mirrors them anymore).
- `strategy_c_vcp_breakout.pine` / `strategy_d_vcp_fixed_bracket.pine` — earlier VCP breakout
  variants, deprecated (no `backend/` module mirrors them anymore).
...
- `p.py` — the production backtest harness (`PortfolioSizedEngine`, using the
  ...
  for strategy semantics — the backend imports directly from this file.
...
- `score.py` — the 0-10 setup-quality score (see `backend/scoring.md`), independent of any
...
- `build_universe.py` — `fetch_candidates()` screens Yahoo Finance for cap/volume/price/
  ...
  universe lives in the DB (`backend/db.py`'s `candidate_tickers` table) and is rebuilt fresh
  on every fetch/compute cycle.
- `static/index.html` — single-page frontend (no build step, no framework); being replaced
  by the React SPA in `frontend/` (see `docs/superpowers/specs/2026-07-31-react-spa-rewrite-design.md`
  and `docs/frontend-docker-setup.md`).
