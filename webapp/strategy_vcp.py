...
def run(df: pd.DataFrame, ind: dict, atr_mult=ATR_MULT, be_trigger_pct=BE_TRIGGER_PCT,
    ...
    if position is not None:
        ...
        open_position = {
            ...
            # (vcp.pine's tp_target %, same level `target` above is computed
...
def evaluate(ticker: str, df: pd.DataFrame, ind: dict | None = None) -> dict:
    """ind: optional pre-computed indicators, shared with strategy_vcpo.py's
    evaluate() by app.py's _compute_one() to avoid recomputing them twice.
