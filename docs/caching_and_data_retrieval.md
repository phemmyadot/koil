...
How the dashboard gets data from Yahoo Finance to the browser, what's cached where, and why. Four separate pieces of state, one shared SQLite database (`backend/db.py`), each with its own staleness rule.

## The database — `backend/db.py`

Everything persistent lives in one file, `backend/app_data.db` (SQLite, WAL mode), across five tables:

| # | Table(s) | What | Owner | TTL / staleness rule |
|---|---|---|---|---|
| 1 | (n/a — `backend/tickers.py` + `universe_meta`) | Ticker universe (which symbols exist at all) | `build_universe.py` / `app.py` | Re-screened at most once/day, automatically |
...
`backend/tickers.py` itself stays a generated `.py` file, not a DB table — it's imported directly (`from backend.tickers import TICKERS`), not deserialized, so `importlib.reload()` works on it. Everything else that used to be separate pickle files (`price_cache.pkl`, `computed_cache.pkl`, `earnings_cache.pkl`) and a plain-text marker (`universe_last_screened.txt`) is now rows in this one DB. See `backend/db_implementation.md` for the full design rationale and schema (that doc is now marked as implemented — this doc describes the as-built result).
...
## 1. Ticker universe — `backend/tickers.py` + `universe_meta` table
...
3. `write_tickers_file()` — overwrites `backend/tickers.py` with the survivors.
...
## 3. Computed strategy results — `backend/app.py`
...
## 4. Earnings dates — `backend/scoring.py`
...
This migration (pickle files → `app_data.db`) is complete on all servers; the one-time `migrate_pickle_to_db.py` script has been removed.

## Docker & deployment

`backend/tickers.py` and the three SQLite files (`app_data.db`, `app_data.db-wal`, `app_data.db-shm` — WAL mode's sidecar files) are:
