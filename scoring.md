...
Runs server-side, in `_compute_one()` alongside the existing `payload["prebreak"] = prebreak.evaluate(...)` call. `strategy` is the `_STRATEGY_MODULES` key (e.g. `"strategy_vcpo"`), and `r` is the in-progress `payload` dict — `r[strategy]` is `{"baseline": {...}, "baseline_config": {...}}`, so stats live one level deeper, under `.get("baseline", {})`.
